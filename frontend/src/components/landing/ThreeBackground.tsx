...
function Particles({ count }: { count: number }) {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      posArray[i3] +=
        (mouseRef.current.x * viewport.width * 0.3 - posArray[i3]) * 0.0003;
      posArray[i3 + 1] +=
        (mouseRef.current.y * viewport.height * 0.3 - posArray[i3 + 1]) *
        0.0003;
...
function CSSBackground() {
  return (
    <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
      ...
      <div
        className="absolute bottom-20 right-1/4 w-72 h-72 bg-gradient-to-br from-violet-500/15 to-pink-500/10 rounded-full blur-3xl animate-float"
        style={{ animationDelay: "-3s" }}
      />
      <div
        className="absolute top-1/2 left-1/3 w-48 h-48 bg-gradient-to-br from-purple-500/10 to-indigo-500/10 rounded-full blur-3xl animate-float-reverse"
